...
# NOTE!! All erro.txt files where for error reporting, didn't really use them on release.


drop_type= ['plain']

#//////// TARGET MODULES [ EDIT FUNCTIONS ]


# Output Method Module:
# Output Type: .csv
# NOTE: This function wasn't really used, it was a test function.
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def output_csv(url,user,passw):
    try:
        csv_file= open('Fang-[Mozilla Pass].csv', 'w') # or use 'wb'
        Fang_csv_writer= csv.writer(csv_file,delimiter=';',quotechar='"',quoting=csv.QUOTE_MINIMAL) 
        Fang_csv_writer.writerow(['url', 'username', 'password'+ '\n'])
        #for data in info:
        Fang_csv_writer.writerow(['%s, %s, %s \n' % (url, user, passw)]) # If you have issues try changing the '[ ]' to '( )'
    
        print(SUCCESS+"Data written to Fang-[Mozilla Pass].csv")
    except EnvironmentError:
        print(ERROR+'EnvironmentError: cannot write data')
...
def output_json(info):
    try:
        with open('Fang-[Mozilla Pass].json', 'w') as json_file:
            json.dump({'password_items':info},json_file)
            print(SUCCESS+"Data written to Fang-[Mozilla Pass].json")
    except EnvironmentError:
        print(ERROR+'EnvironmentError: cannot write data')
...
# CODE: A mild adaptation of the splendid work Renato Alves (@unode) ; firefox_decrypt--> [Github](https://github.com/unode/firefox_decrypt)
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# ///////////// TARGET: My adaptation to add coloring to the plain output text [ on terminal ]
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
boundry= colored('| ---- |','red')
Username= colored('Username: ','cyan')
urlss= colored('Website: ','green')
password= colored('Password: ','cyan')
Header= colored(' \n\n<[--Dumped-Passwords-<<Firefox>>--]>\n====================================== \n','green')
...
class NSSDecoder(object):
    ...
    def find_nss(locations, nssname):
        ...
        for loc in locations:
            ...
            with open('erro.txt','a') as fox_p_erro:
            	fox_p_erro.write('\nFoxError: "Couldn\'t find or load {}. This library is essential to interact with your Mozilla profile."'.format(nssname))
            	fox_p_erro.close()
...
#///////////////  TARGET: Commented default error logging and wrote my own [ error to text file reporting ]
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            for target, error in fail_errors:
             #  LOG.error("Error when loading %s was %s", target, py2_decode(str(error), SYS_ENCODING))
	            with open('erro.txt','a') as fox_p_erro:
	            	fox_p_erro.write('\nFoxError: Error when loading {} was {}\nFoxError: FoxError: <Firefox not installed>.' .format(target, py2_decode(str(error), SYS_ENCODING)))
	            	fox_p_erro.close()
            # removed pass
...
# /////////////////  Changes made to certain LOG calls.
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            else:
...
# /////////////// TARGET [ alot of changes done here..]
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    def decrypt_passwords(self, export, output_format="human", csv_delimiter=";", csv_quotechar="|", format= None):
        """
...
# ////////////// TARGET: After a while playing around with the csv output, i changed the csv.DictWriter 1st *argu from sys.stdout, which just printed the csv formated credentials'
# in terminal, to csv_file which is the format file i created below in line 607 [ AFTER MAKING MY OWN FUNCTION CAUSE I THOUGH THERES WAS WRONG !!, see function format at the top of the script.]
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if output_format == "csv":
            csv_file= open('Fang-[Mozilla Pass].csv', 'w')
            csv_writer = csv.DictWriter(
                csv_file, fieldnames=["url", "user", "password"],
...
# ////////////// TARGET: My adapted csv output method.
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if output_format == "csv":
                ...
                if PY3:
                    # ///////// This scripts default csv_writer.writerow() is the  script's default csv_output method, every thing else was written by me.
                    #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                    try:
                        csv_file= open('Fang-[Mozilla Pass].csv', 'w')
                        pass
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                    ...
                    #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                    try:
                        with open('Fang-[Mozilla Pass].csv', 'wb') as csv_file:
...
# /////////// TARGET: My text file output method addition.
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                for plain_line in raw_output:
                    Half += plain_line


# ////////// TARGET: Full restruture of plain output method to support both text and plain output.
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            else:
                output = (
                    u"\n{0} {1}  {2}\n".format(boundry, urlss, url),
                    u"{0} {1} {2} \n".format(boundry, Username, user),
                    u"{0} {1} {2} \n".format(boundry, password, passw),
                    u"{0}".format(boundry)
                )
                #raw_output = (
                #    "\n| ---- | Website: {}\n".format(url),
...
# //////////// TARGET: My output text file writer.
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if txt_output_method:
            #if os.path.exists(os.path.join('Fang-[Mozilla Pass].txt')):
        ...
        if not csv_output_method and not txt_output_method:
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
        ...
        if csv_output_method:
            ...
            print (WAITING+'Decrypting mozilla credentials....')
            sleep(0.8)
            print (WAITING+'Writting to csv file....')
            sleep (0.8)
            print ('\n')
            print (SUCCESS+"Data written to Fang-[Mozilla Pass].csv")
...
def parse_sys_args():
    ...
    parser.add_argument("-f", "--format", action="store", choices={"csv", "human", "txt"},
